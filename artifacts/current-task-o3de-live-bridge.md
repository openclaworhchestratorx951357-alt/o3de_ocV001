...
## Right now
- active artifact: `entity_parent`
- immediate next artifact: MCP exposure for `entity_parent`
...
- `entity_parent` is the next adjacent read-only entity inspection slice selected from queue/control-file truth
...
Advance from bridge-side `entity_parent` implementation to MCP exposure.
