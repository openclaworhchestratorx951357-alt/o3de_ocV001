...
## Right now
- active artifact: `entity_descendants`
- immediate next artifact: MCP exposure for `entity_descendants`
...
- `entity_ancestors` MCP exposure is complete enough to count as finished
...
- `entity_descendants` is the next adjacent read-only entity inspection slice selected from queue/control-file truth
...
Advance from bridge-side `entity_descendants` implementation to MCP exposure.
